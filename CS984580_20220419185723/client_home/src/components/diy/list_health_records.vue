<template>
	<div class="diy_home diy_list diy_health_records" id="diy_health_records_list">
		<!-- 列表 -->
		<div class="diy_view_list list list-x" v-if="show">
			<router-link class="diy_card goods diy_list_box_wrap" v-for="(o, i) in list" :key="i"
				:to="'/health_records/details?health_records_id=' + o['health_records_id']">
				<!-- 图片 -->
				<div class="diy_list_img_box" v-if="imgList.length" >
					<div class="diy_row" v-for="(item,index) in imgList" :key="item+index" v-show="$check_field('get',item.name,'/health_records/details') && +item.is_img_list">
				...
				<div class="diy_list_item_box">
					<div class="diy_list_item_content" v-for="(item,index) in showItemList" :key="item+index">
						<div class="diy_row" :class="{[item.name]:true}" v-if="$check_field('get',item.name,'/health_records/details') && +item.is_img_list">
							<div class="diy_title">
								<span>{{item.title}}:</span>
							</div>
		...
		<div class="diy_view_table" v-else>
			<table class="diy_table">
				<tr class="diy_row">
					...
					<th class="diy_title" v-if="$check_field('get','date_of_physical_examination')">体检日期
					</th>
					<th class="diy_title" v-if="$check_field('get','doctor')">医生
					</th>
					<th class="diy_title" v-if="$check_field('get','details_of_physical_examination_items')">体检项目详情
					</th>
					<th class="diy_title" v-if="$check_field('get','health_records')">健康档案
					</th>
					<th class="diy_title" v-if="$check_field('get','health_guidelines')">健康指南
					</th>
				</tr>
				<tr class="diy_row" v-for="(o,i) in list" :key="o+i">
					...
					<td class="diy_field diy_date" v-if="$check_field('get','date_of_physical_examination')">
						<span>
							{{ $toTime(o["date_of_physical_examination"] ,"yyyy-MM-dd") }}
						</span>
					</td>
					<td class="diy_field diy_text" v-if="$check_field('get','doctor')">
						<span>
							{{ o["doctor"] }}
						</span>
					</td>
					...
					<td class="diy_field diy_text" v-if="$check_field('get','health_records')">
						<span>
							{{ o["health_records"] }}
						</span>
					</td>
					<td class="diy_field diy_text" v-if="$check_field('get','health_guidelines')">
						<span>
							{{ o["health_guidelines"] }}
						</span>
					</td>
		...
		data() {
			return {
						...
						itemList: [
								...
								{
									title: "体检日期",
									name: "date_of_physical_examination",
									type: "日期",
									is_img_list: "0"
								},
								{
