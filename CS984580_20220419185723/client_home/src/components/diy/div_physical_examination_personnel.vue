<template>
	<div class="diy_details_box diy_div_physical_examination_personnel">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
						...
						<!-- 图片 -->
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/physical_examination_personnel/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "physical_examination_personnel_id",
					source_id: this.obj["physical_examination_personnel_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/physical_examination_personnel/set?physical_examination_personnel_id=' + _this.obj["physical_examination_personnel_id"], {
								praise_len
							}, (res) => {
								if(res.result){
