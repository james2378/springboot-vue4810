<template>
	<div class="diy_details_box diy_div_health_records">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "health_records_id",
					source_id: this.obj["health_records_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/health_records/set?health_records_id=' + _this.obj["health_records_id"], {
								praise_len
							}, (res) => {
								if(res.result){
