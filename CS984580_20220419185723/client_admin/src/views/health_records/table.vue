<template>
	<el-main class="bg table_wrap">
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			...
			<el-table-column prop="date_of_physical_examination" label="体检日期"
				v-if="user_group == '管理员' || $check_field('get','date_of_physical_examination')" min-width="200">
                <template slot-scope="scope">
                	{{ $toTime(scope.row["date_of_physical_examination"],"yyyy-MM-dd") }}
                </template>
			</el-table-column>
			<el-table-column prop="doctor" label="医生"
				v-if="user_group == '管理员' || $check_field('get','doctor')" min-width="200">
			</el-table-column>
			...
			<el-table-column prop="health_records" label="健康档案"
				v-if="user_group == '管理员' || $check_field('get','health_records')" min-width="200">
			</el-table-column>
			<el-table-column prop="health_guidelines" label="健康指南"
				v-if="user_group == '管理员' || $check_field('get','health_guidelines')" min-width="200">
			</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/health_records/table','set') || $check_action('/health_records/view','set') || $check_action('/health_records/view','get') || $check_action('//table','add') || $check_action('//view','add')" >
